...
la gestione delle configurazioni di sistemi distribuiti. La tipologia di
repository che possono essere ‘agganciati’ prevede:
...
di configurazione vengono caricati a partire da un classpath locale (file).
...
**Le configurazioni e le proprietà dei singoli moduli applicativi possono essere
dinamicamente utilizzando l’annotation \@RefreshScope sul componente
interessato.**
...
tokens ai resource services di back-end .
...
Stiamo utilizzando il Password credentials grant type per la user authorization
(ad oggi non è stato implementato nessun componente che la utilizza) e Client
Credentials grant per l’ authorization dei microservizi.
...
un single entry point sul sistema IL, utilizzato per il routing delle singole
...
Su VGI-IL viene utilizzato il componente Zuul di Netflix opensource inserito nel
framework Spring. In questa prima versione Zuul viene utilizzato per il routing
delle richieste e viene abilitato mediante l’annotation \@EnableZuulProxy.
...
Eureka implementa il client-side discovery pattern, cioè è la parte client che è
responsabile di determinare la service location per le varie istanze presenti di
ogni servizio e gestire così anche il load-balancing tra di esse.
...
![](media/a10fc37c7d4cfb7b0582e7ce55fac735.png)
...
monitoring dashboard dell’intero sistema.
...
con le direttive per la build della propria docker image. Qui di seguito
l’esempio per il modulo *auth-service*:
